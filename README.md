# Escape Game Numérique - SAÉ S5.01 Développement Avancé

Projet réalisé dans le cadre de la SAÉ S5.01, en méthode agile, sue le thème imposé d'un **Escape Game** intégrant une dimension éthique, écologique ou durable (lien avec R5.A.13 — Économie durable et numérique).

## Groupe
- Groupe 303-304
- Membres : BOUKHIMA Reda,DUMEZ Apollinaire, PEREIRA DE BARROS Tiago, PERITO Maïssane
- Enseignants référents : Jérome Fessy, Laurent Guitignano

## Mission 0 — Audit de compétences et constitution de l'équipe
 
Chaque membre du groupe présente ci-dessous sa compétence de spécialité et ses compétences de couverture, justifiées à partir de son retour d'expérience de stage.

### Maïssane
 
- **Compétence de spécialité :** Backend, en particulier l'interfaçage avec des éléments physiques (capteurs / microcontrôleurs)
  *Justification :* Projet personnel antérieur de contrôle de la qualité de l'air avec un ESP32, démontrant une expérience concrète de connexion entre le monde physique et une solution logicielle — compétence directement transférable à la contrainte « interaction avec le monde physique via capteurs ou microcontrôleurs » du sujet.
- **Compétences de couverture :** Frontend / conception de site web, gestion de la relation avec un client non technique
  *Justification :* Stage effectué au sein d'une association, avec réalisation complète d'un site internet en PHP mis en production chez un hébergeur. Élaboration du site de A à Z en autonomie, en traduisant les besoins d'une interlocutrice sans compétences informatiques en un produit fonctionnel et adapté.
- **Soft skills clés :** Travail en autonomie, communication client (traduire un besoin non technique en solution technique)

### Apollinaire
 
- **Compétence de spécialité :** Base de données, Backend 
  *Justification :* compétence acquise durant mon stage de 2ème année en traitement de la donnée à Airbus dans le secteur du Manufacturing Engineering, démontrant un savoir-faire et une rigueur particulière à adopter sur ce projet
- **Compétences de couverture :** Gestion de projet / Agile, Documentation
  *Justification :* apprentissage de la méthodologie Agile en 2ème année, j'ai été très intéressé par cette organisation professionnelle et j'ai pu la mettre en pratique durant mon stage de 2ème dans de certaines mesures. Commpétence clé afin de bien pouvoir communiquer et progresser sur un projet comme celui-ci
- **Soft skills clés :** autonomie, organisation, clarté

### Reda

- **Compétence de spécialité :** Backend, bases de données et architectures client-serveur.
  *Justification :* Expérience professionnelle validée lors d'un stage suivi d'un CDD chez Servair, avec le développement d'une application de logistique de catering aérien. Pour ce projet, j'ai notamment mis en place des Server-Sent Events avec Spring Boot pour gérer les flux de données, ce qui répond directement à la contrainte de la SAÉ exigeant une solution robuste basée sur une architecture distribuée via API.
- **Compétences de couverture :** Recueil de besoin, Frontend (JavaScript natif, HTML5, CSS3).
  *Justification :* Le recueil de besoin et l'analyse ont été des étapes centrales lors du développement de l'application logistique réalisé durant le stage et la refonte du site de l'association, me permettant de bien structurer le projet avant de coder.
- **Soft skills clés :** Adaptabilité, capacité d'analyse et de traduction des besoins, autonomie et rigueur technique.

### Tiago
 
- **Compétence de spécialité :** Frontend, Test
  *Justification :* Développement et tests de composants React et réalisation de tests BFF, avec la réalisation de tickets durant mon stage de 2ème année au sein de l'équipe Okapi du Groupe La Poste.
- **Compétences de couverture :** Backend
  *Justification :* Découverte du fonctionnement du backend et de ses liens avec le frontend, la base de données, le cache et la gateway durant mon stage de 2ᵉ année au sein de l’équipe Okapi du Groupe La Poste.
- **Soft skills clés :** autonomie, organisation


# Répartition des rôles (garants de domaine)
 
| Domaine              | Garant(e)       | Backup(s)       |
|-----------------------|-----------------|------------------|
| Frontend              |                 |                  |
| Backend               |                 |                  |
| Base de données       |                 |                  |
| IoT / Capteurs        |                 |                  |
| Architecture distribuée (API / Message Broker) |  |  |
| Tests / Qualité       |                 |                  |
| Documentation         |                 |                  |
| Gestion de projet / Agile |             |                  |
 
*Note : ces rôles ne sont pas figés — l'objectif est d'assurer une couverture de tous les piliers du projet, pas de cloisonner les membres.*
 
## Jalons
 
| Jalon | Échéance | Livrable |
|-------|----------|----------|
| Jalon 1 — Conception | Fin septembre | Game Design Document (GDD) |
| Jalon 2 — Prototype | Mi-novembre | MVP (Minimum Viable Product) |
| Jalon 3 — Beta | Mi-janvier | Version intégrée + rapport d'équipe |
| Jalon 4 — JPO | Mi-février | Produit final + exposition |
 
## Contraintes techniques
 
- Solution robuste : frontend, backend, base de données
- Architecture distribuée (API ou Message Broker)
- Interaction avec le monde physique via capteurs ou microcontrôleurs
- Respect des paradigmes de qualité, tests par sprint, documentation
 
